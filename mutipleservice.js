...
module.controller("GetUserDetails",['$scope','$http','$log','$getUserDetails', '$getReposDetails', function($scope, $http , $log, $getUserDetails, $getReposDetails){             
// if I am not using Any angular in-built or custom services, i can directly create a function // module.controller("GetUserDetails", function($scope){});              
    ...
    // $http.get("https://api.gitHub.com/users/" + $scope.repo + '/repos').then(function(response){
                      $getReposDetails.getData($scope.repo,function(response){
